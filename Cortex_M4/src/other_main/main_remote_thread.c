...
K_THREAD_STACK_DEFINE(thread_1_area, APP_TASK_STACK_SIZE);
static struct k_thread thread_1;

K_THREAD_STACK_DEFINE(thread_2_area, APP_TASK_STACK_SIZE);
...
void insert_buff(void *arg1, void *arg2, void *arg3){
	...
	while(1){
		...
		for(k=0;k<495;k+=2){
			if(my_spi_read(spi, read_conf, djcb_rx, 2)){
				printk("Read error \n");
			}
			else {
...
void main(void){
	...
	receive_message(&msg,&len);
	k_thread_create(&thread_1, thread_1_area,  K_THREAD_STACK_SIZEOF(thread_1_area),insert_buff, NULL, NULL, NULL, 2, 0, K_NO_WAIT);
	k_thread_create(&thread_2, thread_2_area,  K_THREAD_STACK_SIZEOF(thread_2_area),send_buff, NULL, NULL, NULL, 2, 0, K_NO_WAIT);
